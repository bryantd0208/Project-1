...
#     state = node.state
#     for actions in problem.actions(state):
#         new_state = problem.result(state, actions)
#         cost = node.path_cost
#         cost = cost + problem.action_cost(state, actions, new_state)
#         yield Node(new_state, node, actions, cost)

    state = node.state
    for action in problem.actions(state):
        s1 = problem.result(state, action)
        cost = node.path_cost + problem.action_cost(state, action,s1)
    ...
    #########################################################
       # this line of code can be uncommented and used
...
def depth_first_search(problem):
    ...
    open = PriorityQueue([(0, start)]) #frontier
    visited = set()
    while open:
        cost, current = open.pop()
        if problem.goal_test(current.state):
            return current
        visited.add(current.state)
        for child in current.expand(problem):
            if not child.state in visited:
                open.add((-child.path_cost, child))
...
#     reached = set()
#     nr_expanded_DepthFS = 0
#     while frontier:
#         node = frontier.pop()
#         nr_expanded_DepthFS += 1
#         if problem.goal_test(node.state):
#             print("Nr. of nodes expanded for Depth-First Search: " , nr_expanded_DepthFS)
#             return node
...
#     h = h(start, open)
    ...
    while open:
        ...
        for child in current.expand(problem):
            if not child.state in visited or child.path_cost < visited[child.state].path_cost:
                open.add((h(child), child))
...
#     h = h(start, open)
    ...
    while open:
        ...
        for child in current.expand(problem):
            if not child.state in visited or child.path_cost < visited[child.state].path_cost:
                open.add((h(child) + child.path_cost, child))
    ########################################################
    # return node   # this line of code can be uncommented and used
